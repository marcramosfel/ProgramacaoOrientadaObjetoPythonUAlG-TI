...
class Adamastor:
    ...
    def listar_produtos_respetivos_fornecedores(self):
        sql = '''SELECT fornecedores.NomeDaEmpresa, produtos.NomeDoProduto, fornecedores.CódigoDoFornecedor 
              FROM fornecedores INNER JOIN produtos ON produtos.CódigoDoFornecedor = fornecedores.CódigoDoFornecedor 
              order by fornecedores.CódigoDoFornecedor'''
    ...
    def maior_emcomenda_valor(self):
        sql = """SELECT 
                    CódigoDaEncomenda, SUM(PreçoUnitário * Quantidade)
                FROM
                    detalhes_da_encomenda
                GROUP BY CódigoDaEncomenda
                HAVING SUM(PreçoUnitário * Quantidade) = (SELECT 
                        SUM(PreçoUnitário * Quantidade) AS total
                    FROM
                        detalhes_da_encomenda
                    GROUP BY CódigoDaEncomenda
                    ORDER BY total DESC
